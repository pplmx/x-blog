...
- 🎯 **SEO 优化** - Open Graph、JSON-LD 结构化数据，以及覆盖全部已发布文章（无上限）与每个系列的完整站点地图（DEC-318）
- ⬆️ **文章置顶** - 将重要文章置顶显示
...
- ⏰ **定时文章上线通知** - 以"已发布但 publish_at 在未来"方式保存的文章（编辑日历）在真正上线的那一刻自动通告：其系列/分类/标签的关注者各收到恰好一条新文章通知（收件箱 + 推送 + 开启邮件的也收邮件），由该文章上线后的第一次公开读取触发——即使多 worker 并发也保证恰好一次（DEC-336）
- 🌐 **读者语言的通知文案** - 登录读者的收件箱标题、通知邮件与每周精选邮件按其选择的语言（en 或 zh）渲染——语言切换器会把选择同步到账号，英文读者切换后立即告别「系列更新 / 新文章发布」的中文行与中文邮件；从未切换的读者保持原有中文文案（DEC-338）
