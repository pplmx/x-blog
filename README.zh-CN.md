...
- 🚫 **屏蔽读者** - 读者互相关注与 @提及 通知从未有「拉黑」的开关：登录读者现在可以在对方的公开主页直接屏蔽另一位评论者（单向且不可见——对方不会收到任何提示、也可以继续评论），被屏蔽者的 @提及、回复、线程评论与关注动态将不再送达自己；`/account` 会列出已屏蔽读者，一键取消屏蔽即可恢复通知（DEC-425, TASK-437）。
- 📧 **访客邮箱订阅讨论** - 「订阅讨论」此前只对登录读者开放：想只订阅某一篇
  文章讨论的访客除了注册别无他路。文章评论头部新增一个紧凑表单，访客输入邮箱、
  点开双重确认链接（同订阅页脚模式：对外无探测、每条新记录只发一封确认邮件）
  后，每条评论通过审核就收到一封讨论邮件——深链到该评论、自带每条订阅专属的
  一键退订链接，可从邮箱一键停止后续讨论邮件。全程无需注册账号（DEC-427, TASK-438）。
- 📧 **`/admin/newsletter` 周报运维面板** - 每周摘要（读者 + 访客）此前只有后端机器、没有管理界面：管理员能用 CLI 或超管接口触发，却看不到谁订阅了周报、上次何时发出。新增的面板展示读者与访客的周报订阅数、上次发送时间、下一期窗口将包含多少篇文章，并提供 dry-run **预览**（send-weekly?dry_run=true，不实际发送）与超管确认的 **立即发送**——在既有触发接口旁补齐了监控与预览闭环（DEC-423, TASK-436）
