...
- 📱 **响应式设计** - 完美适配移动端
- 🔒 **管理后台** - 内置内容管理后台
- 🧪 **完善测试** - 4200+ 个测试 (后端 1879 + Nuxt 前端 2405), 后端覆盖率 93.7%
...
- 📧 **讨论周报选项** - 用邮箱订阅讨论此前意味着「每条通过的评论都发一封邮件」，
  同时跟多篇讨论时很吵。确认页上访客可勾选「每周汇总一次」：逐条评论的邮件
  立即停止，每周摘要任务（与订阅周报同一 CLI/管理员触发）会按每篇被跟的讨论
  发一封汇总邮件，列出当周通过审核的新评论（深链到评论、同样带一键退订）——
  订阅早已拥有的「每周汇总」选项，如今也开放给讨论（DEC-429, TASK-439）。
