...
describe('Pagination', () => {
  it('should not render when totalPages is 1 or less', () => {
    const { container } = render(<Pagination currentPage={1} totalPages={1} baseUrl="/" />);
  ...
  it('should not show previous button on first page', () => {
    const { container } = render(<Pagination currentPage={1} totalPages={5} baseUrl="/" />);

    expect(container.textContent).not.toContain('上一页');
  });

  it('should not show next button on last page', () => {
    const { container } = render(<Pagination currentPage={5} totalPages={5} baseUrl="/" />);
