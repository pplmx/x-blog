...
const mermaidRender = vi.fn().mockResolvedValue({ svg: '<svg data-testid="mermaid-svg">test diagram</svg>' });
const mockMermaid = {
  initialize: vi.fn(),
...
describe('MarkdownContent', () => {
  ...
  describe('Mixed content', () => {
    it('renders paragraphs and code blocks together', async () => {
      const wrapper = mountMarkdown('<p>Before code</p>\n\n```ts\ncode here\n```\n\n<p>After code</p>');
      await flushPromises();
    ...
    it('renders paragraphs and images together', async () => {
      const wrapper = mountMarkdown('<p>See image below</p>\n<img src="img.png" alt="img" />\n<p>After image</p>');
      await flushPromises();
