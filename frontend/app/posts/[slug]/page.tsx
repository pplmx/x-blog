    ...
    <>
      ...
      <header className="mb-8">
        ...
        <div className="text-gray-500">
          <span>{date}</span>
          {post.category && (
            <span className="ml-4 px-2 py-1 bg-gray-100 rounded text-sm">{post.category.name}</span>
          )}
        </div>
        <div className="flex gap-2 mt-4">
          {post.tags.map((tag: Tag) => (
            <span key={tag.id} className="text-sm px-3 py-1 bg-blue-50 text-blue-600 rounded">
              {tag.name}
            </span>
          ))}
        </div>
