...
test("mermaid and math render on post page", async ({ page }) => {
  ...
  for (let i = 0; i < mermaidCount; i++) {
    ...
    console.log(`Mermaid div ${i} - has content: ${hasContent}, has SVG: ${content.includes('<svg')}`);
  ...
  for (let i = 0; i < mathCount; i++) {
    ...
    const hasContent = content.trim().length > 0;
    console.log(`Math span ${i} - has content: ${hasContent}, has katex: ${content.includes('katex')}`);
