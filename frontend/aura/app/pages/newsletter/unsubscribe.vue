<!--
  Guest newsletter unsubscribe (DEC-351, TASK-401).

  Landed on via the `?token=` link inside every newsletter email (the
  per-subscriber token only those emails carry). Fires the backend unsubscribe
  once on the token, then shows a clear success/invalid state. No form: the
  token IS the action — clicking the emailed link is the consent revocation,
  so we do not ask for confirmation or credentials (the subscriber has no
  account to log into).

  Route lives at /newsletter/unsubscribe (flat, sibling of confirm.vue under
  the same directory with no parent newsletter.vue — no Nuxt nesting issue).
-->
...
useSeo(() => ({
	title: t("newsletter.unsubscribe.seoTitle"),
	description: t("newsletter.unsubscribe.seoDesc"),
...
async function run() {
	...
	fired = true;
	try {
		const { unsubscribeNewsletter } = await import("~~/api/public/newsletter");
		await unsubscribeNewsletter(token.value);
		state.value = "done";
	} catch (e) {
		// A 404 = unknown token (what a random guess gets; also what a
		// never-subscribed address gets). Anything else (no response:
		// unreachable backend) is a NETWORK condition — the token may still be
		// valid, so we must not tell the holder their link is spent.
		const status =
