...
describe("sitemap.xml route", () => {
  it("should fetch from backend /sitemap.xml", async () => {
    ...
    expect(fetchCalls[0].options.headers).toEqual({ Accept: "application/xml" });
  });
...
describe("RSS feed route", () => {
  it("should fetch from backend /rss/feed.xml", async () => {
    ...
    expect(fetchCalls[0].options.headers).toEqual({ Accept: "application/rss+xml" });
  });
...
describe("Atom feed route", () => {
  it("should fetch from backend /rss/atom.xml", async () => {
    ...
    expect(fetchCalls[0].options.headers).toEqual({ Accept: "application/atom+xml" });
  });
