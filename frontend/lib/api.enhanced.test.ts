...
describe('API - Enhanced Features', () => {
  ...
  describe('fetchRelatedPosts', () => {
    it('fetches related posts for a given post', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(`${API_BASE}/api/posts/1/related?limit=5`);
    });
  ...
  describe('incrementLikes', () => {
    it('increments likes for a post', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(`${API_BASE}/api/posts/1/like`, { method: 'POST' });
    });
  });

  describe('fetchPopularPosts', () => {
    it('fetches popular posts with limit', async () => {
      const mockPosts = [{ id: 1, title: 'Popular', slug: 'popular', views: 1000 }];
      ...
      expect(result).toEqual(mockPosts);
      expect(global.fetch).toHaveBeenCalledWith(`${API_BASE}/api/posts/popular/list?limit=10`);
    });
  });
});
