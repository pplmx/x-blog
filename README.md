...
- ✍️ **Author follow** - at a multi-editor blog a reader who loved one writer's posts subscribes to just that person: a follow button sits on every post byline AND the writer's public `/authors/{id}` archive header (the only follow path for a writer with nothing published yet), and every new post from that author lands in the follower's inbox as a durable row plus a Web Push — the person-shaped cousin of the category/series/tag follows (rounds 353/355)
